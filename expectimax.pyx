...
from logic cimport check_game, get_possible_states, up, down, left, right
from heuristics cimport evaluate


cpdef double expectimax(list board, int depth):
    cdef double sum2, sum4
    cdef list twos, fours
    if depth == 0 or check_game(board) == -1:
        ...
        for state2 in twos:
            sum2 += expectimax(state2, depth-1)
        for state4 in fours:
            sum4 += expectimax(state4, depth-1)
